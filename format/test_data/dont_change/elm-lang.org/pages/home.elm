module Main exposing (Feature, Model, Msg(..), Quote, SmallQuote(..), assetsChart, container, copyRight, downloadLink, elmTitle, featureImage, featureQuote, featureText, features, fixedPointer, footer, grouped, guideButton, init, main, movingText, navColumn, navTitle, navitem, onMouseMove, pageColumn, performanceChart, smallQuotes, sources, subContainer, subscriptions, tangram, toplevel, tryButton, update, view, viewLarge, viewMedium, viewSmall)
    ...
    case msg of
            ...
            { model
                | logo =
                    if Logo.isMoving model.logo then
                        ...
                        model.logo
                , taglines =
                    if TextAnimation.isMoving model.taglines then
                        TextAnimation.step model.taglines
        ...
        TimePassed ->
            { model
                ...
                , quotes =
                    if model.justSelected then
                        model.quotes

                    else
                        Cycle.step model.quotes
                , justSelected = False
            }
...
subscriptions model =
    Sub.batch
        ...
        , case model.visibility of
            ...
            E.Visible ->
                if Logo.isMoving model.logo || TextAnimation.isMoving model.taglines then
                    E.onAnimationFrameDelta TimeDelta

                else
                    Time.every 3000 (\_ -> TimePassed)
        ]
            ...
            <|
                case quote of
                    ...
                    QuoteByEmployee text author gitlink title company link ->
                        ...
                        , case link of
                            Just url ->
                                E.link [ F.size 16 ] { url = url, label = E.text company }

                            Nothing ->
                                E.el [ F.size 16 ] (E.text company)
                        ]
...
guideButton =
    Ui.linkButton "https://guide.elm-lang.org"
        "Guide"
