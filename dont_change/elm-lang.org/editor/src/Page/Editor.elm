    ...
    case msg of
        ...
        OnTogglePackages ->
            let
                ...
                push =
                    if isOpenNow then
                        Ui.ColumnDivider.pushLeft

                    else
                        Ui.ColumnDivider.pushRight
            in
            ( { model
...
view model =
    let
        packageStyles =
            if model.isPackageUiOpen then
                if Window.isLessThan model.window Ui.Package.width then
                    [ style "max-width" Ui.Package.widthPx, style "border" "0" ]

                else
                    [ style "max-width" Ui.Package.widthPx ]
    ...
    main_
        ...
        [ Ui.ColumnDivider.view OnDividerMsg
            model.window
            model.divider
