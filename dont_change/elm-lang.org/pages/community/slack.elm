module Main exposing (Model, Msg(..), Outcome(..), Status(..), decoder, decoderHelp, errorToOutcome, init, isWaiting, main, update, view, viewContent, viewStatus)
...
decoderHelp isOk =
    if isOk then
        D.succeed InvinteSent

    else
        D.map errorToOutcome (D.field "error" D.string)
