...
viewStacked area ( areas, lines, dots ) =
    let
        opacity =
            "opacity: " ++ String.fromFloat (Area.opacityContainer area)
...
viewArea { system, lineConfig, area } line_ style_ interpolation data_ =
    let
        ...
        commands first middle last =
            Utils.concat
                [ Path.Move (ground <| Path.toPoint first), Path.Line (Path.toPoint first) ]
                interpolation
                [ Path.Line (ground <| Path.toPoint last) ]
    in
    Utils.viewWithEdges interpolation <|
        \first middle last ->
