...
step index char state =
  let lowerName start = LowerName start (fromIntegral (index - start))
   in case (state, char) of
        (Start, ElmChar.M_) ->
        ...
        (InUpperName start, ElmChar.Space) ->
          Commit [UpperName start (fromIntegral (index - start)), Space]
        (Start, ElmChar.E_) ->
        ...
        (Ambiguous start Ambiguous.P, ElmChar.OpenParentheses) ->
          Commit [lowerName start, OpenParentheses]
        ...
        (Ambiguous start Ambiguous.Po, ElmChar.Equals) ->
          Commit [lowerName start, Equals]
        (InLowerName start, ElmChar.CloseParentheses) ->
          Commit [lowerName start, CloseParentheses]
        (InUpperName start, ElmChar.Newline) ->
          Commit [UpperName start (fromIntegral (index - start)), Newline]
        (Start, ElmChar.Zero) ->
        ...
        (InLowerName start, ElmChar.Newline) ->
          Commit [lowerName start, Newline]
        (InUpperName start, ElmChar.Equals) ->
          Commit [UpperName start (fromIntegral (index - start)), Equals]
        ...
        (InHex start, ElmChar.Space) ->
          Commit [Number start (fromIntegral (index - start)), Space]
        ...
        (InHex start, ElmChar.Newline) ->
          Commit [Number start (fromIntegral (index - start)), Newline]
