...
static int is_multiline_list_pattern_node(int node) {
  int item;
  int start = 0;
  while (get_list_item(node, &item, &start) == 0) {
    if (is_multiline_pattern_node(item) || has_multiline_left_comment(item) ||
...
static int is_multiline_function_call_pattern_node(int node) {
  int item;
  int start = 0;
  while (get_argument(node, &item, &start) == 0) {
    if (is_multiline_pattern_node(item) || has_multiline_left_comment(item) ||
...
static int simple_pattern_parse(int *node) {
  return qualified_name_parse(node) && lower_name_parse(node) &&
         ...
         with_comments_in_parentheses_pattern_parse(node) && int_parse(node) &&
         triple_string_parse(node) && normal_string_parse(node) &&
